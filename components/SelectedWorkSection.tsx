    ...
    <section className="section-pad-big bg-white">
      <div className="section-wid pb-5">
        ...
        <div className="flex items-center justify-between border-t border-b border-gray-200 py-3 md:py-4">
          ...
          <div className="flex items-center gap-2 ml-4 shrink-0">
            <button onClick={() => sliderRef.current?.slickPrev()} className="w-8 h-8 md:w-9 md:h-9 border border-gray-300 flex items-center justify-center hover:border-black transition-colors">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <button onClick={() => sliderRef.current?.slickNext()} className="w-8 h-8 md:w-9 md:h-9 border border-gray-300 flex items-center justify-center hover:border-black transition-colors">
      ...
      <div className="section-wid mt-6 md:mt-8">
        <Slider ref={sliderRef} {...settings}>
          {filtered.map((p) => (
            <div key={p.num}>
              <div className="cursor-pointer group pr-4">
                ...
                <div className="flex items-start justify-between py-4 mt-4 border-t border-gray-200">
                  <div>
